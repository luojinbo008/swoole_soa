...
class MySQLi implements Swoole\IDatabase
{
    ...
    protected function tryReconnect($call, $params)
    {
        ...
        for ($i = 0; $i < 2; $i++) {
            ...
            if ($result === false) {
                if ($this->mysqli->errno == 2013 or $this->mysqli->errno == 2006 or ($this->mysqli->errno == 0
                        and !$this->mysqli->ping())) {
    ...
    {
        $result = $this->tryReconnect([$this->mysqli, 'query'], [$sql, $resultmode]);
        if (!$result) {
    ...
    public function multi_query($sql)
    {
        ...
        while (call_user_func_array(['parent', 'more_results'], []) && call_user_func_array([
                'parent', 'next_result'
            ], [])) {

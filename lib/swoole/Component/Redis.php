...
class Redis
{
    ...
    public function getIncreaseId($appKey, $init_id = 1)
    {
        ...
        if ($this->_redis->exists($main_key)) {
            ...
            if (empty($inc)) {
                \Swoole\Swoole::$php->log->put("redis::incr() failed. Error: ".$this->_redis->getLastError());
                return false;
            }
            return $inc;
        } else if($this->_redis->getLastError()) { //上面的if条件返回false,可能是有错误，或者key不存在，这里要判断下
            return false;
        } else { // 这才是说明key不存在，需要初始化
            $init = $this->_redis->set($main_key, $init_id);
            if ($init == false) {
                \Swoole\Swoole::$php->log->put("redis::set() failed. Error: ".$this->_redis->getLastError());
                return false;
            } else {
                return $init_id;
            }
    ...
    public function connect()
    {
        try {
            ...
            \Swoole\Swoole::$php->log->error(__CLASS__ . " Swoole Redis Exception" . var_export($e, 1));
            return false;
        }
    }
    ...
    public function __call($method, $args = [])
    {
        ...
        while (1) {
            try {
                ...
                \Swoole\Swoole::$php->log->error(__CLASS__ . " [" . posix_getpid() . "] Swoole Redis[{$this->config['host']}:{$this->config['port']}]

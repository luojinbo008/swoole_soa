...
class RPCServer extends Base implements Swoole\IFace\Protocol
{
    ...
    protected $appNamespaces    = [];       // 应用程序命名空间
    protected $ipWhiteList      = [];       // IP白名单
    protected $userList         = [];       // 用户列表
    ...
    public function onReceive($serv, $fd, $reactor_id, $data)
    {
        ...
        if ($request === false) {
            ...
            if ($ret === false) {
                trigger_error("SendToClient failed. code=" . $this->server->getLastError()
                    ." params=" . var_export($request, true) . "\nheaders=" . var_export($_header, true), E_USER_WARNING);
            }

...
class SelectDB
{
    ...
    public function where()
    {
        ...
        if (count($args) == 1) {
            ...
            $where = '`' . str_replace('`', '', $field) . '`' . $this->db->quote($expr) . " '"
                . $this->db->quote($value) . "'";
        }
    ...
    public function getone($field = '')
    {
        ...
        if ($this->auto_cache or !empty($cache_id)) {
            $cache_key = empty($cache_id) ? self::CACHE_PREFIX . '_one_' . md5($this->sql) : self::CACHE_PREFIX
                . '_all_' . $cache_id;
    ...
    public function getall()
    {
        ...
        if ($this->enableCache) {
            ...
            if (empty($this->cacheOptions['object_id'])) {
                $cacheObject = $php->cache;
            } else {
                $cacheObject = $php->cache($this->cacheOptions['object_id']);
            }
